...
export interface CheckpointStore {
  save(id: string, data: { results: any[]; pinnedCardIds?: string[] }): Promise<void>;
  load(id: string): Promise<{ results: any[]; pinnedCardIds?: string[] } | null>;
}

export class FileCheckpointStore implements CheckpointStore {
  private dir: string;
  constructor() {
    this.dir = path.join(os.tmpdir(), "foundry-iq-kb-checkpoints");
    fs.mkdirSync(this.dir, { recursive: true });
  }
  async save(id: string, data: { results: any[]; pinnedCardIds?: string[] }): Promise<void> {
  ...
  async load(id: string): Promise<{ results: any[]; pinnedCardIds?: string[] } | null> {
    validateCheckpointId(id);
    const filePath = path.join(this.dir, `${id}.json`);
    if (!path.resolve(filePath).startsWith(path.resolve(this.dir) + path.sep)) {
      throw new Error("Invalid checkpoint path");
